...
public sealed class StorageUIController : UIController, IOnSystemChanged<StorageSystem>
{
    ...
    public StorageWindow CreateStorageWindow(StorageBoundUserInterface sBui)
    {
        ...
        window.OnCraftButtonPressed += () => OnCraftButtonPressed(window); // stalker-changes
        window.OnDisassembleButtonPressed += () => OnDisassembleButtonPressed(window); // stalker-changes
    ...
    private void OnCraftButtonPressed(StorageWindow window)
    {
    ...
    private void OnDisassembleButtonPressed(StorageWindow window)
    {
        if (window.StorageEntity is not { } storageEnt)
            return;
        EntityManager.RaisePredictiveEvent(new DisassembleStartedEvent(EntityManager.GetNetEntity(storageEnt)));
    }
